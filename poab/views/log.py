...
def log_view(request):
    ...
    for log in pages_list[curr_page]:
        ...
        if len(log.images) > 0:
            ...
            gallerylink = """<span class="image_icon"><a title="Show large images related to this entry" href="/view/log/%s/0"></a></span>""" % (
                log.id)
        else:
            gallerylink = ''
        # print log.tracks
    ...
    </div></div><span class="imagedescription">%s</span></div>''' % (
                    '/static', image.location.replace('/srv', ''), image.name, image.title, '/static',
                    image.location.replace('/srv', ''), '500/', image.name, image.alt, flickr_name,
                    image.image_flickr_ref[0].photoid, image.comment)
            else:
                inlineimage = '''<div class="log_inlineimage"><div class="imagecontainer"><a href="%s%s%s" title="%s" rel="image_colorbox" ><img class="inlineimage" src="%s%s%s%s" alt="%s" /></a><div class="caption">
        <span>&#8594;</span>
            <a href="http://www.flickr.com/%s/%s" target="_blank">www.flickr.com</a>
    </div></div></div>''' % ('/static', image.location.replace('/srv', ''), image.name, image.title, '/static',
                             image.location.replace('/srv', ''), '500/', image.name, image.alt, flickr_name,
                             image.image_flickr_ref[0].photoid)  # TODO breaks when no flickr-info in db
        ...
        class Logdetails(object):
            def __init__(self, topic, twitter, guid, localtime, published, content, rounded_distance, timezone,
                         timespan, country, continent, lasttrkpt, infomarker, log, gallerylink, author):
                self.topic = topic
        ...
        logdetails = Logdetails(log.topic, twitter, guid, localtime, log.published, log_content_display,
                                rounded_distance, timezone, timespan, country, continent, log.trackpoint_log_ref,
                                log.trackpoint_log_ref, log, gallerylink,
                                log.author)  # TODO: "log.trackpoint_log_ref, log.trackpoint_log_ref" was originally "infomarker, lasttrkpt"
